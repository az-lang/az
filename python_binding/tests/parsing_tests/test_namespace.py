...
def test_enum_like_classes(
    ...
    enum_like_classes_field_names = {
        ...
        if (
            is_class_final(cls)
            and len(field_names := list(to_self_referential_class_fields(cls)))
            != 0
        )
    }
...
def test_constructible_classes(
    ...
    assert [
        (
            cls,
            to_sorted_lists_diff(
                sorted_property_names, sorted_parameter_names
            ),
        )
    ...
    assert [
        (
            ...
            (
                ...
                else to_lists_different_elements(parameter_names, field_names)
            ),
        )
        for cls, signature in constructible_class_signatures.items()
        if (
            ...
            or (
                ...
                != (
                    field_names := [
                        ...
                        if (
                            to_base_annotation(field_annotation)
                            is not ClassVar
                        )
                    ]
                )
...
def test_expression_subclasses(
    module: ModuleType, module_namespace: dict[str, Any]
) -> None:
    subclasses = module.Expression.__subclasses__()
...
def test_parsing_error_subclasses(
    module: ModuleType, module_namespace: dict[str, Any]
) -> None:
    subclasses = module.ParsingError.__subclasses__()
...
def test_statement_subclasses(
    module: ModuleType, module_namespace: dict[str, Any]
) -> None:
    subclasses = module.Statement.__subclasses__()
