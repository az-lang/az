...
pub(crate) struct PyToken(Token<TokenOwnedString>);
...
impl From<PyToken> for Token<TokenOwnedString> {
    fn from(value: PyToken) -> Self {
        value.0
    }
}

impl From<Token<TokenOwnedString>> for PyToken {
...
impl Repr for Token<TokenOwnedString> {
    fn repr(&self, py: Python<'_>) -> PyResult<String> {
        Ok(format!(
            "{}({}, position={})",
            PyToken::NAME,
