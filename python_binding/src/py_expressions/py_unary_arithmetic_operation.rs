...
use crate::py_unary_arithmetic_operators::UnaryArithmeticOperatorWrapper;
...
pub(crate) struct PyUnaryArithmeticOperation(
    UnaryArithmeticOperation<OwnedString>,
);
            ...
            PyTuple::new_bound(
                ...
                [
                    OwnedExpressionWrapper::from(
                        self.0.operand.as_ref().clone(),
                    )
                    .into_py(py),
                    UnaryArithmeticOperatorWrapper::from(self.0.operator)
                        .into_py(py),
                ],
            ),
            kwargs,
...
impl From<UnaryArithmeticOperation<OwnedString>>
    for PyUnaryArithmeticOperation
{
    fn from(value: UnaryArithmeticOperation<OwnedString>) -> Self {
        Self(value)
    }
}

impl From<PyUnaryArithmeticOperation>
...
impl Repr for UnaryArithmeticOperation<OwnedString> {
