...
pub(crate) struct PyTuple(Tuple<OwnedString>);

#[pymethods]
impl PyTuple {
    #[new]
    #[pyo3(signature = (
    elements, /, *, open_parenthesis_position, comma_positions,
    ...
    fn __getnewargs_ex__<'py>(
        &'py self,
        py: Python<'py>,
    ) -> PyResult<(Bound<'py, pyo3::types::PyTuple>, Bound<'py, PyDict>)> {
...
impl From<Tuple<OwnedString>> for PyTuple {
    fn from(value: Tuple<OwnedString>) -> Self {
        Self(value)
    }
}

impl From<PyTuple> for Tuple<OwnedString> {
...
impl Repr for Tuple<OwnedString> {
