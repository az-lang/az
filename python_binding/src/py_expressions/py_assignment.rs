...
pub(crate) struct PyAssignment(Assignment<OwnedString>);
            ...
            PyTuple::new_bound(
                ...
                [
                    OwnedExpressionWrapper::from(
                        self.0.target.as_ref().clone(),
                    )
                    .into_py(py),
                    OwnedExpressionWrapper::from(
                        self.0.value.as_ref().clone(),
                    )
                    .into_py(py),
                ],
            ),
...
impl From<Assignment<OwnedString>> for PyAssignment {
    fn from(value: Assignment<OwnedString>) -> Self {
        Self(value)
    }
}

impl From<PyAssignment> for Assignment<OwnedString> {
...
impl Repr for Assignment<OwnedString> {
