use pyo3::{
    pyclass, pymethods, Bound, Py, PyAny, PyClassInitializer, PyErr, PyObject,
...
#[pyclass(
module = "az.tokenization", name = "NumericLiteralValueUnexpectedCharacter",
extends = PyLexicalError, frozen
)]
pub(crate) struct PyNumericLiteralValueUnexpectedCharacter(
    NumericLiteralValueUnexpectedCharacter<TokenOwnedString>,
);

#[pymethods]
impl PyNumericLiteralValueUnexpectedCharacter {
    #[getter]
    ...
    #[getter]
    fn kind(&self, py: Python<'_>) -> Py<PyNumericLiteralValueKind> {
        PyNumericLiteralValueKind::from_rust(self.0.kind, py)
    }
    ...
    #[new]
    #[pyo3(signature = (character, expected, kind, position, string, /))]
    fn new(
        character: char,
        expected: String,
...
impl From<NumericLiteralValueUnexpectedCharacter<TokenOwnedString>>
    for PyNumericLiteralValueUnexpectedCharacter
{
    fn from(
...
impl From<PyNumericLiteralValueUnexpectedCharacter> for PyErr {
    fn from(value: PyNumericLiteralValueUnexpectedCharacter) -> Self {
        Self::new::<PyNumericLiteralValueUnexpectedCharacter, _>((
            value.character(),
            value.expected(),
            value.kind(unsafe { Python::assume_gil_acquired() }),
            value.position(),
            value.string(),
        ))
    }
...
impl Repr for NumericLiteralValueUnexpectedCharacter<TokenOwnedString> {
    fn repr(&self, py: Python<'_>) -> PyResult<String> {
        Ok(format!(
            "{}({}, {}, {}, {}, {})",
            PyNumericLiteralValueUnexpectedCharacter::NAME,
...
impl Repr for PyNumericLiteralValueUnexpectedCharacter {
    fn repr(&self, py: Python<'_>) -> PyResult<String> {
        self.0.repr(py)
    }
}
