use pyo3::{
    pyclass, pymethods, Bound, PyAny, PyClassInitializer, PyErr, PyObject,
    PyResult, PyTypeInfo, Python,
};

use az::tokenization::NumericLiteralValueUnexpectedCharacter;

use crate::macros::impl_unordered_rich_cmp_for_derived_py_class;
use crate::py_numeric_literal_value_kind::PyNumericLiteralValueKind;
use crate::py_substring_position::PySubstringPosition;
use crate::traits::{Repr, RichCmp};
use crate::types::TokenOwnedStr;

use super::py_lexical_error::PyLexicalError;

#[derive(Clone, PartialEq)]
#[pyclass(
module = "az.tokenization", name = "NumericLiteralValueUnexpectedCharacter",
extends = PyLexicalError, frozen
)]
pub(crate) struct PyNumericLiteralValueUnexpectedCharacter(
    NumericLiteralValueUnexpectedCharacter<TokenOwnedStr>,
);

#[pymethods]
impl PyNumericLiteralValueUnexpectedCharacter {
    #[getter]
    fn character(&self) -> char {
        self.0.character
    }

    #[getter]
    fn expected(&self) -> String {
        self.0.expected.as_ref().into()
    }

    #[getter]
    fn kind(&self) -> PyNumericLiteralValueKind {
        self.0.kind.into()
    }

    #[getter]
    fn position(&self) -> PySubstringPosition {
        self.0.position.clone().into()
    }

    #[getter]
    fn string(&self) -> String {
        self.0.string.as_ref().into()
    }

    #[new]
    #[pyo3(signature = (character, expected, kind, position, string, /))]
    fn new(
        character: char,
        expected: String,
        kind: PyNumericLiteralValueKind,
        position: PySubstringPosition,
        string: String,
    ) -> PyClassInitializer<Self> {
        PyLexicalError::new().add_subclass(Self(
            NumericLiteralValueUnexpectedCharacter {
                character,
                expected: expected.into(),
                kind: kind.into(),
                position: position.into(),
                string: string.into(),
            },
        ))
    }

    fn __repr__(&self, py: Python<'_>) -> PyResult<String> {
        self.repr(py)
    }

    fn __richcmp__(
        &self,
        other: &Bound<'_, PyAny>,
        op: pyclass::CompareOp,
        py: Python<'_>,
    ) -> PyResult<PyObject> {
        self.rich_cmp(other, op, py)
    }
}

impl From<NumericLiteralValueUnexpectedCharacter<TokenOwnedStr>>
    for PyNumericLiteralValueUnexpectedCharacter
{
    fn from(
        value: NumericLiteralValueUnexpectedCharacter<TokenOwnedStr>,
    ) -> Self {
        Self(value)
    }
}

impl From<PyNumericLiteralValueUnexpectedCharacter> for PyErr {
    fn from(value: PyNumericLiteralValueUnexpectedCharacter) -> Self {
        Self::new::<PyNumericLiteralValueUnexpectedCharacter, _>((
            value.character(),
            value.expected(),
            value.kind(),
            value.position(),
            value.string(),
        ))
    }
}

impl Repr for NumericLiteralValueUnexpectedCharacter<TokenOwnedStr> {
    fn repr(&self, py: Python<'_>) -> PyResult<String> {
        Ok(format!(
            "{}({}, {}, {}, {}, {})",
            PyNumericLiteralValueUnexpectedCharacter::NAME,
            self.character.repr(py)?,
            self.expected.repr(py)?,
            self.kind.repr(py)?,
            self.position.repr(py)?,
            self.string.repr(py)?,
        ))
    }
}

impl Repr for PyNumericLiteralValueUnexpectedCharacter {
    fn repr(&self, py: Python<'_>) -> PyResult<String> {
        self.0.repr(py)
    }
}

impl_unordered_rich_cmp_for_derived_py_class!(
    PyNumericLiteralValueUnexpectedCharacter,
    PyLexicalError
);
