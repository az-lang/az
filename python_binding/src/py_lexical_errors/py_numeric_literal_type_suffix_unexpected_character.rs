...
impl PyNumericLiteralTypeSuffixUnexpectedCharacter {
    ...
    #[getter]
    fn value_kind(&self, py: Python<'_>) -> Py<PyNumericLiteralValueKind> {
        PyNumericLiteralValueKind::from_rust(self.0.value_kind, py)
    }
...
impl From<PyNumericLiteralTypeSuffixUnexpectedCharacter> for PyErr {
    fn from(value: PyNumericLiteralTypeSuffixUnexpectedCharacter) -> Self {
        Self::new::<PyNumericLiteralTypeSuffixUnexpectedCharacter, _>((
            ...
            value.string(),
            value.value(),
            value.value_kind(unsafe { Python::assume_gil_acquired() }),
        ))
    }
}

impl Repr for NumericLiteralTypeSuffixUnexpectedCharacter<TokenOwnedString> {
