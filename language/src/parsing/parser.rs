...
use super::filler::{Filler, FillerVec};
...
use super::statements::{ExpressionStatement, Statement};
    ...
    > Parser<StringType, TokenStringType, Tokens>
{
    ...
    pub(super) fn into_fillers(self) -> FillerVec<StringType> {
            ...
            match token.content {
                TokenContent::CommentBlock(value) => {
                    self.fillers.push(Filler {
                        content: FillerContent::CommentBlock(value.into()),
                        position: token.position,
                    });
                }
                TokenContent::CommentLine(value) => {
                    self.fillers.push(Filler {
                        content: FillerContent::CommentLine(value.into()),
                        position: token.position,
                    });
                }
                ...
                TokenContent::Whitespace(value) => {
                    self.fillers.push(Filler {
                        content: FillerContent::Whitespace(value.into()),
                        position: token.position,
                    });
                }
                _ => {
    ...
    > Parser<StringType, TokenStringType, Tokens>
{
        ...
        while let Some(next_token) = parser.peek() {
            if let Ok(operator) = BinaryOperator::try_from(&next_token.content)
            {
                ...
                result = if matches!(operator, BinaryOperator::Call) {
                    let (operand, next_parser) = parser
                    ...
                    match operator {
                        ...
                        BinaryOperator::LessThan => Expression::BinaryComparison(BinaryComparison {
                            left: Box::new(result),
                            right: Box::new(operand),
                            operator: BinaryComparisonOperator::LessThan,
                            operator_position,
                            operator_fillers,
                        }),
        ...
        if let Ok(operator) = UnaryOperator::try_from(&token.content) {
            ...
            match token.content {
                TokenContent::Identifier(string) => match string.as_ref() {
                    ...
                    WHILE_LOOP_OPENER => {
                        self.parse_while_loop(token.position, fillers)
                    }
                    _ => Ok((
                        Expression::Identifier(Identifier {
                            string: string.into(),
    ...
    fn parse_block(
        ...
        open_brace_fillers: FillerVec<StringType>,
    ) -> Result<
        (Expression<StringType>, Self),
        ParsingError<StringType, TokenStringType>,
    > {
    ...
    fn parse_conditional(
        mut self,
        antecedent_opener_position: SubstringPosition,
        antecedent_opener_fillers: FillerVec<StringType>,
    ) -> Result<
        (Expression<StringType>, Self),
        ParsingError<StringType, TokenStringType>,
    > {
            ...
            Ok((
                Expression::UnidirectionalConditional(
                    UnidirectionalConditional {
                        antecedent: Box::new(antecedent),
                        consequent,
                        opener_position: antecedent_opener_position,
                        opener_fillers: antecedent_opener_fillers,
                    },
                ),
                parser,
            ))
    ...
    fn parse_function_definition(
        ...
        opener_fillers: FillerVec<StringType>,
    ) -> Result<
        (Expression<StringType>, Self),
        ParsingError<StringType, TokenStringType>,
    > {
        ...
        let (close_parenthesis_position, signature_closing_fillers) = loop {
            match token.content {
                ...
                _ => {
                    ...
                    if let Expression::AnnotatedIdentifier(parameter) =
                        expression
                    {
                    ...
                    match next_token.content {
                        TokenContent::Comma => {
                            comma_fillers.push(next_fillers);
                            comma_positions.push(next_token.position);
    ...
    fn parse_tuple_or_grouping(
        ...
        open_parenthesis_fillers: FillerVec<StringType>,
    ) -> Result<
        (Expression<StringType>, Self),
        ParsingError<StringType, TokenStringType>,
    > {
        ...
        match next_token.content {
            ...
            _ => {
                ...
                match token.content {
                    ...
                    TokenContent::Comma => {
                        let mut comma_fillers = vec![fillers];
                        let mut comma_positions = vec![token.position];
...
{
    type Item = (Token<TokenStringType>, FillerVec<StringType>);
